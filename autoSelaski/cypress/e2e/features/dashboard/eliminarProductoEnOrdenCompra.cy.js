// ============================
// Archivo: eliminarProductoEnOrdenCompra.cy.js
// Proyecto: autoSelaski - QA Automatización Cypress
// Descripción: Prueba E2E para eliminar productos en órdenes de compra
// ============================

import EliminarProductoEnOrdenCompraPage from '../../pages/eliminarProductoEnOrdenCompraPage';

describe('Dashboard QA - Importación - Eliminar Producto en Orden Compra', { 
  viewportHeight: 1080, 
  viewportWidth: 1920 
}, () => {

  const eliminarProductoPage = new EliminarProductoEnOrdenCompraPage();
  ...
  const generarProductoParaEliminar = () => {
    ...
    return {
      ...
      posicion: '10',
      sku: `QA-COMPRA-DELETE-${randomId}-${Date.now()}`,
      codigoProveedor: `PROV-DELETE-${randomId}`,
  ...
  const crearProductoParaEliminar = (producto) => {
    ...
    // Navegación
    eliminarProductoPage.clickImportacion();
    eliminarProductoPage.clickOrdenCompra();
  ...
  afterEach(function() {
    if (this.currentTest.state === 'failed') {
      ...
      cy.screenshot(`error-eliminar-compra-${this.currentTest.title}-${Date.now()}`);
    }
  });
});